...
{
    uint8_t col = x;  // 这里假设 x 是以像素为单位
    uint8_t page = y;
    
    // 每个汉字占用16x16像素，分为上下两部分各16字节
    uint8_t *glyph_top = (uint8_t*)&Hzk[2 * no];      // 上半部分字形数据
    uint8_t *glyph_bottom = (uint8_t*)&Hzk[2 * no + 1]; // 下半部分字形数据
    
    // 显示上半部分
    OLED_SetPosition(page, col);
    OLED_WriteNBytes(glyph_top, 16);  // 写入上半部分16个字节

    // 显示下半部分
    OLED_SetPosition(page + 1, col);  // 列地址不变
    OLED_WriteNBytes(glyph_bottom, 16);  // 写入下半部分16个字节
}
